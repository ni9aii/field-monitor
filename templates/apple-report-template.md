...
> **Кросс-проверка:** независимые медиа могут публиковать сообщения о блокировке
> соответствующих сервисов. Замеры field-monitor фиксируют активный блок
> (RST, code 0, ~8 с таймаут) независимо от внешних публикаций.
...
```
target,apple,203.0.113.5,30,200,132,open,16,-,-
...
```
| EXAMPLE | example-host | github | Some(0) | Some(8000) | open | HTTPS_FAIL |
```

- `HTTPS = Some(0)` — нет HTTP-ответа, соединение сброшено;
- `Latency = Some(8000)` — таймаут на уровне TLS (~8 с);
...
- `HTTPS latency` — задержка HTTPS, мс (`Some(8000)` ≈ 8 с, таймаут TLS-handshake);
...
> (см. раздел 2). В текущем окне apple виден как OK на большинстве точек.
